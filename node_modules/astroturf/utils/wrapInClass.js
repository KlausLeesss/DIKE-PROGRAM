...
function wrapInClass(text) {
  ...
  match = null; // eslint-disable-next-line no-cond-assign

  while (match = rComposes.exec(text)) {
    composes.push(match[0]);
  }

  text = text.replace(rImports, '').replace(rComposes, ''); // comment prevents Sass from removing the empty class

  let val = _commonTags.stripIndents`
    .cls1 {
      ${text.trim() || '/*!*/'}
